...
void initializeApplication(void)
{
	...
	for (int i=0; i<numRows; i++) {
		...
		locks[i] = (pthread_mutex_t*)calloc(numCols, 
	...
	doorAssign = (int*)malloc(numDoors*sizeof(int));
	...
	for (int i=0; i<numBoxes; i++) {
		...
		while (test > -1) {
			...
			//test box against door coordinates
			while (j < numDoors) {
			...
			//test box against box coordinates
			j = 0;
			while ((j < i) && (test > 0)) {
			...
			//if no duplicates, test successful, save box
			if (j == i) test = -1;
		}
	...
	for (int i=0; i<numBoxes; i++) {
		...
		robotLoc[i][0],robotLoc[i][1],roboBox->assignedDoor->doorID);
		if (i == numBoxes-1) fprintf(fp,"\n");
	}
