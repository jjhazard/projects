...
void initializeApplication(void)
{
	...
	map = (bool**) malloc(numRows * sizeof(bool*));
	locks = (pthread_mutex_t**)calloc(numRows, sizeof(pthread_mutex_t*));
	
	for (int i=0; i<numRows; i++) {
		grid[i] = (int*)calloc(numCols, sizeof(int));
		map[i] = (bool*)calloc(numCols, sizeof(bool));
		locks[i] = (pthread_mutex_t*)calloc(numCols, sizeof(pthread_mutex_t));
		//initialize locks
	...
	scene->fp = fp;
	scene->numBots = numBoxes;
	scene->gridlocks = locks;
	deadLocked = (bool*)calloc(numCols, sizeof(bool));
	scene->deadlocked = deadLocked;
	pthread_mutex_init(&scene->filelock, NULL);
	pthread_mutex_init(&scene->DLlock, NULL);
    ...
    for (int i=0; i<numDoors; i++)
    {
        ...
        theDoor->doorID = i;
        theDoor->doorX = doorLoc[i][1]; //1 is column index of doorLoc, 0 is row
